...
export class PanelPlaceHandler extends MouseHandler {
    ...
    move({ curPoint, viewPortData, setViewPortData, appActions, appData, keys }) {
        ...
        for (const curShape of this.panels) {
            ...
            if (curShape.vertical) dy = 0; else dx = 0
            let newX = x + dx
            let newY = y + dy

            if(curShape.canBePlaced(newX, newY, minDist)){
                curShape.setPosition(newX, newY);
                for (let joint of curShape.jointFromBackSide) {
                    joint.setLength(joint.getLength() + (dx + dy))
                }
                for (let joint of curShape.jointFromFrontSide) {
                    joint.setLength(joint.getLength() - (dx + dy))
