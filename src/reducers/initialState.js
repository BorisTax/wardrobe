...
function createNewState({ wardrobe = { width: 2400, height: 2200, depth: 600 } }) {
  const leg = 30
  const panels = [
    new PanelShape({ id: 0, wardrobe, name: "Крыша", length: wardrobe.width, vertical: false, position: { x: 0, y: wardrobe.height - 16 }, fixed_move: true, fixed_minlength: true, fixable: true, deletable: false, gabarit: true }),
    ...
    new PanelShape({ id: 3, wardrobe, name: "Стойка боковая", length: wardrobe.height - 32 - leg, vertical: true, position: { x: wardrobe.width - 16, y: 16 + leg }, fixed_move: true, fixed_minlength: true, fixable: true, deletable: false, gabarit: true }),
  ]
